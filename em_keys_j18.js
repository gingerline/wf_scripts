...
async function reloadData() {
    ...
    if (em_keys && em_keys.length > 0) {
        ...
        em_keys.forEach((em_key) => {
            ...
            clonedElement.find("#key-name").text(em_key.tokenId);// replace with name

            // Update the div with ID "key-value" within the cloned element
            clonedElement.find("#key-value").text(em_key.token);

            clonedElement.attr('em-keyId', em_key.tokenId);

            clonedElement.find(".action-link-table").attr('em-keyId', em_key.tokenId);
            clonedElement.find(".action-link-table").attr('em-keyName', em_key.tokenId);// replace with name
