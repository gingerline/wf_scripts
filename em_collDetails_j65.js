...
let VECTOR_API_URL = ""; // This will be set dynamically
let pageQuery = 0; // Global variable to keep track of the current page
...
async function loadAndDisplayVectors(url, page) {
    const em_vectors = await fetchVectorsAndHandleResponse(url, page);
    await reloadVectorData(em_vectors);
    managePaginationButtons(em_vectors);
}

async function resetQueryFormClickHandler(event) {
    const queryForm = document.getElementById("wf-form-Query-Vector");
    queryForm.reset();
    pageQuery = 0; //reset pageQuery to 0 for new request
    // Find the input element with the id topK
    let topKInput = document.getElementById("topK");
    // Check if the input element exists before setting its value
    if (topKInput) {
        topKInput.value = 2;
    }
    ...
    VECTOR_API_URL = API_URL + '/vectors/getAll';
    pageQuery = 0;// Reset to the first page for a new query
    await loadAndDisplayVectors(VECTOR_API_URL, pageQuery);
    //document.getElementById("reset-button-div").style.display = "none";
    const resetButtonDiv = document.getElementById("reset-button-div");
    // To hide
    resetButtonDiv.classList.add("not-visible");


}

// Pagination Control Functions
async function nextVectorsClickHandler(event) {
    pageQuery++;
    await loadAndDisplayVectors(VECTOR_API_URL, pageQuery);
}

async function prevVectorsClickHandler(event) {
    if (pageQuery > 0) pageQuery--;
    loadAndDisplayVectors(VECTOR_API_URL, pageQuery);
}


function managePaginationButtons(em_vectors) {
    const prevBtn = document.getElementById("prev-btn");
    const nextBtn = document.getElementById("next-btn");

    // Helper function to toggle 'disabled' class
    const toggleDisabled = (button, shouldBeDisabled) => {
        if (shouldBeDisabled) {
    ...
    const handleClick = (event) => {
        // Directly check if the button has the 'disabled' class
    ...
    // Determine whether to disable or enable buttons
    //const isPrevDisabled = !em_vectors || em_vectors.length === 0 || pageQuery <= 0;
    const isPrevDisabled = pageQuery <= 0;
    const isNextDisabled = (!em_vectors || em_vectors.length === 0 || em_vectors.length < 10) || VECTOR_API_URL.includes('query');

    toggleDisabled(prevBtn, isPrevDisabled);
    toggleDisabled(nextBtn, isNextDisabled);
}
...
Webflow.push(function () {
    ...
    $('form').submit(function (e) {
        ...
        if (formId === "wf-form-Delete-Collection") {
            ...
            pageQuery = 0; // Reset the pageQuery for a new query
            VECTOR_API_URL = BASE_URL + collectionId + '/query/text'; // Update VECTOR_API_URL for query
            finalActionURL = VECTOR_API_URL;
            finalData = JSON.stringify({
        ...
        $.ajax(finalActionURL, {
            ...
            .done((res) => {
                ...
                if (formId === "wf-form-Query-Vector") {
                    //document.getElementById("reset-button-div").style.display = "block";
                    const resetButtonDiv = document.getElementById("reset-button-div");
                    // To hide
                    resetButtonDiv.classList.remove("not-visible");

                    pageQuery = 0; // Reset to the first page for a new query
                    VECTOR_API_URL = finalActionURL; // set the correct URL to vectorAPI url
                    //await loadAndDisplayVectors(queryURL, pageQuery);
                    reloadVectorData(res[0]);
                    managePaginationButtons(res[0]); // Manage pagination buttons after query
                } else {
                    //document.getElementById("reset-button-div").style.display = "none";
                    const resetButtonDiv = document.getElementById("reset-button-div");
                    // To hide
                    ...
                    document.getElementById("wf-form-Query-Vector").reset();
                    reloadAllData();
                }
...
async function fetchVectorsAndHandleResponse(URL, page = 0) {
    try {
        if (!outsetaJWT) {
            outsetaJWT = getOutsetaKey() || getParam('access_token');
        }
...
async function reloadVectorData(em_vectors) {
    ...
    const prevNextDiv = document.getElementById("nav-button-div");
    ...
    if (em_vectors && em_vectors.length > 0) {
        ...
        em_vectors.forEach((em_vec) => {
            ...
            if (em_vec.distance) {
                ...
                clonedElement.find("#vector-score").hide();
                clonedElement.find("#vector-score-block").hide();
                clonedElement.find("#vector-score-block-prev").hide();
            }
        ...
        prevNextDiv.style.display = "grid";
        ...
        if (pageQuery == 0) {
            prevNextDiv.style.display = "none";
        }
    }
...
async function reloadAllData() {
    ...
    if (em_coll) {
        ...
        if (em_coll.config.model === "OPEN-AI") {
            emCollCard.find("#coll-model").text("Open AI Embeddings");
        } else if (em_coll.config.model === "HUGGINGFACE-MINI-LM-L6") {
            emCollCard.find("#coll-model").text("Mini LM L6");
        } else if (em_coll.config.model === "CUSTOM") {
            emCollCard.find("#coll-model").text("Custom");
        }
        ...
        // hide and show the elements and set defaults
    ...
    // Set VECTOR_API_URL for initial data load
    VECTOR_API_URL = BASE_URL + collectionId + '/vectors/getAll';
    pageQuery = 0;
    await loadAndDisplayVectors(VECTOR_API_URL, pageQuery);
    ...
    document.getElementById("next-btn").addEventListener("click", nextVectorsClickHandler);
    document.getElementById("prev-btn").addEventListener("click", prevVectorsClickHandler);
