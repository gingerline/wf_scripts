...
const COLLECTION_API_URL = BASE_URL + "collections";
const COLLECTION_DETAILS_URL = "https://emno-v1.webflow.io/app/collections-details?collectionId=";
...
async function reloadData() {
    ...
    async function fetchDataAndHandleResponse(URL) {
        try {
            // adding here just in case before fetch
            if (!outsetaJWT) {
                let outsetaJWT = getOutsetaKey();
                if (!outsetaJWT) {
                    outsetaJWT = getParam('access_token');
    ...
    // get data
    const em_collections = await fetchDataAndHandleResponse(COLLECTION_API_URL);
    ...
    if (em_collections && em_collections.length > 0) {
        ...
        em_collections.forEach((em_coll) => {
            ...
            let collectionHref = COLLECTION_DETAILS_URL + em_coll.id;
            clonedElement.find("#coll-name").attr("href", collectionHref);
            ...
            // Update the div with ID "coll-dimension" within the cloned element
            clonedElement.find("#coll-dimension").text(em_coll.config.dim);

            // Update the div with ID "coll-dimension" within the cloned element
            clonedElement.find("#coll-id").text(em_coll.id);
