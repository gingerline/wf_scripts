...
function managePaginationButtons(em_vectors) {
    ...
    // Helper function to toggle 'disabled' class and click behavior
    const toggleDisabled = (button, shouldBeDisabled) => {
        if (shouldBeDisabled) {
            button.classList.add("disabled");
            button.classList.add("not-visible");
            // Directly bind the preventClick function for disabled state
            button.onclick = preventClick;
        } else {
            button.classList.remove("disabled");
            button.classList.remove("not-visible");
            // Reset to default behavior when not disabled
            button.onclick = null; // or button.onclick = yourDesiredFunction;
        }
    };

    const preventClick = (event) => {
        // Check if the button has 'disabled' class and prevent default action
        if (event.currentTarget.classList.contains("not-visible")) {
