...
function managePaginationButtons(em_vectors) {
    ...
    // Attach a single click listener to handle clicks for both buttons
    document.addEventListener('click', function (event) {
        let targetElement = event.target;

        // Check if the target element itself is a disabled anchor
        if (targetElement.tagName === "A" && targetElement.classList.contains("disabled")) {
            event.preventDefault();
            event.stopPropagation();
            return;
        }

        // Check if the parent element is a disabled anchor
        let parentElement = targetElement.parentElement;
        if (parentElement && parentElement.tagName === "A" && parentElement.classList.contains("disabled")) {
            event.preventDefault();
            event.stopPropagation();
        }
    });
