...
function managePaginationButtons(em_vectors) {
    ...
    const navButtonDiv = document.getElementById("nav-button-div");

    // Hide both buttons if there are no results
    if (!em_vectors || em_vectors.length === 0) {
        prevBtn.style.display = "none";
        nextBtn.style.display = "none";
        navButtonDiv.style.display = "none";
        return;
    }
    ...
    // Show or hide the 'next' button based on the number of results
    if (em_vectors.length < 10) {
        nextBtn.style.display = "none";
    } else {
        nextBtn.style.display = "inline";
    }

    // Show the nav-button-div if either button is visible
    navButtonDiv.style.display = (prevBtn.style.display === "inline" || nextBtn.style.display === "inline") ? "block" : "none";
